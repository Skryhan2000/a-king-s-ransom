    ...
    public void finishWork() {
        ...
        if (isInputValid()) {
            try {
                switch (action) {
                    case "create":
                        if ((Boolean) CommandProvider.getCommandProvider().getCommand("CREATE_ORDER").execute(orderData)) {
                        ...
                        break;
                    case "update":
                        if ((Boolean) CommandProvider.getCommandProvider().getCommand("UPDATE_ORDER").execute(orderData)) {
                            WorkWithAlert.getInstance().showAlert("Обновления данных",
                                    "Успех", "Данные сохранены", dialogStage, "INFORMATION");
                            goBack();
                        }
