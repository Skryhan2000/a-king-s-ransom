...
public class ReceiveAllProduct implements Command<Transmission,ConcurrentHashMap<Integer, Product>> {
    @Override
    public ConcurrentHashMap<Integer, Product> execute(Transmission request) {
        return ServiceFactory.getInstance().getProductService().receiveAllElem();
    }
}
