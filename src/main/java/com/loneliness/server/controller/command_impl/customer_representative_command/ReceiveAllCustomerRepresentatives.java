...
public class ReceiveAllCustomerRepresentatives implements Command<Transmission,ConcurrentHashMap<Integer,CustomerRepresentative> > {
    @Override
    public ConcurrentHashMap<Integer, CustomerRepresentative> execute(Transmission request) {
        return ServiceFactory.getInstance().getCustomerRepresentativeService().receiveAllElem();
    }
}
